  ...
  if (options.fallback) {
    before.push(function (req, res) {
      if (res.statusCode === 404) {
        fs.stat(options.fallback.address, function (err, stats) {
          if (stats.isDirectory()) {
            options.fallback.address = path.join(options.fallback.address, 'index.html');
          }

          fs.readFile(options.fallback.address, 'binary', function (err, file) {
            if (err) {
